...
fun TaskEditDialog(
    ...
    val checklistItems = remember { mutableStateListOf<ChecklistItem>().also { it.addAll(task.checklist) } }
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                if (task.imageUrls.isNotEmpty()) {
                    ImageAttachmentRow(imageUrls = task.imageUrls)
                }
                    ...
                    if (title.isNotBlank()) {
                        onSave(
                            task.copy(
                                ...
                                checklist = checklistItems.filter { it.text.isNotBlank() }.toList(),
                            )
                        )
                        onDismiss()
                    }
